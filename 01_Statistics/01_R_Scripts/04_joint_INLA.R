...
p_fixed <- ggplot(fixed, aes(x = mean, y = param)) +
  ...
  labs(
    title = "Joint INLA fixed effects — Process 2 (Buxbaumia viridis)",
...
print(p_fixed)
if (save_fig) {
  ggsave(file.path(fig_dir_joint, "fixed_effects_P2.png"),
...
p_beta <- ggplot(beta_s_df, aes(x, density)) +
  ...
  labs(
    title = "Posterior distribution of β_s",
...
print(p_beta)
if (save_fig) {
  ggsave(file.path(fig_dir_joint, "beta_s_posterior.png"),
...
pred_distrib <- ggplot(res$pred, aes(x = p_buxbaumia, fill = factor(P_Bux))) +
  ...
  facet_wrap(~Site) +
  scale_fill_manual(
    values = c("0" = "#888780", "1" = "#6161d9"),
    labels = c("Absent", "Present")
  ) +
  labs(
    title = "Distribution of predicted probabilities by site",
...
if (save_fig) {
  ggsave(file.path(fig_dir_joint, "predictions_distribution.png"),
...
p_roc <- ggplot(roc_df, aes(x = 1 - specificity, y = sensitivity)) +
  ...
  labs(
    title = "ROC curve — joint INLA model",
...
print(p_roc)
if (save_fig) {
  ggsave(file.path(fig_dir_joint, "ROC_curve.png"),
...
cat(paste0(
  "Valid CPO values : ", length(cpo_clean), " / ", length(cpo_binom), "\n",
...
p_pit <- ggplot(pit_df, aes(x = pit)) +
  ...
  geom_hline(
    yintercept = expected_height,
    linetype = "dashed", color = "#6161d9"
  ) +
  labs(
    title = "PIT — calibration (Buxbaumia observations only)",
    x = "PIT", y = "Count"
  ) +
  theme_minimal()
...
print(p_pit)
if (save_fig) {
  ggsave(file.path(fig_dir_joint, "PIT_calibration.png"),
...
for (k in 1:nlevels(res$data$Site)) {
  ...
  p_w1 <- ggplot(grid, aes(X, Y, fill = w1)) +
    ...
    geom_point(
      data = df_k, aes(X, Y, shape = factor(P_Bux)),
...
if (save_fig) {
  ggsave(file.path(fig_dir_joint, "pred_all_sites.png"),
...
print(p_voronoi)
if (save_fig) {
  ggsave(file.path(fig_dir_joint, "voronoi.png"),
