...
for (s in sites) {
  ...
  X_tmp  <- ppp(x = pts_df$X, y = pts_df$Y,
  ...
  p <- ggplot(ppp_df, aes(x = x, y = y, color = marks, shape = marks)) +
    geom_polygon(data = win_df, aes(x = x, y = y), 
...
for (s in sites){
  ...
  p_pres <- ggplot() +
    ...
    geom_point(data = df_pres, aes(x = X, y = Y),
               color = "firebrick", size = 1.5, alpha = 0.7) +
    coord_equal() + theme_bw() +
    labs(title = paste0(s, " - Colonised substrates (P_Bux = 1)"),
         x = "X", y = "Y")
  ...
  p_dens <- ggplot(dens_df, aes(x = x, y = y, fill = value)) +
    ...
    geom_polygon(data = win_df, aes(x = x, y = y),
                 inherit.aes = FALSE, fill = NA, color = "black", linewidth = 0.5) +
    geom_point(data = df_site, aes(x = X, y = Y),
               inherit.aes = FALSE, size = 0.4, alpha = 0.5) +
    scale_fill_viridis_c() + coord_equal() + theme_bw() +
    labs(title = paste0(s, " - Local deadwood density (sigma = 10 m)"),
         x = "X", y = "Y", fill = "Scaled\ndensity")
  
  # A(u) : store mean/sd before scaling
  X_Z      <- all_pts ; marks(X_Z) <- df_site$Z
  ...
  p_alt <- ggplot(alt_df, aes(x = x, y = y, fill = value)) +
    ...
    geom_polygon(data = win_df, aes(x = x, y = y),
                 inherit.aes = FALSE, fill = NA, color = "black", linewidth = 0.5) +
    scale_fill_viridis_c(option = "magma") + coord_equal() + theme_bw() +
    labs(title = paste0(s, " - Smoothed altitude"),
         x = "X", y = "Y", fill = "Scaled\naltitude")
  
  # S(u) : store mean/sd before scaling
  X_S        <- all_pts ; marks(X_S) <- df_site$Sap_max
  ...
  p_sap <- ggplot(sap_df, aes(x = x, y = y, fill = value)) +
    ...
    geom_polygon(data = win_df, aes(x = x, y = y),
                 inherit.aes = FALSE, fill = NA, color = "black", linewidth = 0.5) +
    scale_fill_viridis_c(option = "cividis") + coord_equal() + theme_bw() +
    labs(title = paste0(s, " - Smoothed saproxylic index"),
         x = "X", y = "Y", fill = "Scaled\nsap. index")
  ...
  results_ppm <- bind_rows(results_ppm,
                           data.frame(
                             Site      = s,
                             Term      = names(coef_ppm),
  ...
  p_resid <- ggplot(res_df, aes(x = x, y = y, color = value)) +
    ...
    coord_equal() + theme_bw() +
    labs(title = paste0(s, " - PPM residuals"),
  ...
  p_pred <- ggplot(pred_df, aes(x = x, y = y, fill = value)) +
    ...
    coord_equal() + theme_bw() +
    labs(title = paste0(s, " - PPM predicted intensity"),
